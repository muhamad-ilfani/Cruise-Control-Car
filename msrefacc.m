...
if distance <= D_des && v_rel <= 0
    Vx_des = v_x2+ades_fol_old+ades_fol*Ts; %Follow mode
    q
elseif distance < D_des+d_off && v_rel > 0 && ades_crs > ades_fol
    Vx_des = v_x2+ades_fol_old+ades_fol*Ts; %Follow mode
    w
elseif distance < D_des+d_off && v_rel > 0 && ades_crs <= ades_fol
    Vx_des = v_x2+ades_crs_old+ades_crs*Ts; %Cruise Mode
    D_des = distance;
    e
elseif distance > D_des && v_rel < 0 && ades_crs > ades_fol
    Vx_des = v_x2+ades_fol_old+ades_fol*Ts; %Follow mode
    r
elseif distance > D_des && v_rel < 0 && ades_crs <= ades_fol
    Vx_des = v_x2+ades_crs_old+ades_crs*Ts; %Cruise Mode
    D_des = distance;
    y
elseif distance >= D_des+d_off && v_rel >= 0
    Vx_des = v_x2+ades_crs_old+ades_crs*Ts; %Cruise Mode
    D_des = distance;
    o
end
